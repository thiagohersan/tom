...
RSpec.describe DuelsController, type: :controller do
  ...
  describe 'POST #create' do
    ...
    it 'returns http success' do
      post :create, params: { user_id: ApplicationHelper::encrypt(user.id) }
    ...
    it 'returns trends in a random order' do
      ...
      post :create, params: { user_id: ApplicationHelper::encrypt(user2.id) }
    ...
    context 'user leaves application' do
      it 'returns unanswered duels' do
        duel = create(:duel)
        post :create, params: { user_id: ApplicationHelper::encrypt(duel.user_id) }
  ...
  describe "PATCH #update" do
    ...
    it 'fails if duel does not belong to user' do
      user = create(:user)
      duel = create(:duel)
      patch :update, params: { id: duel.id, user_id: ApplicationHelper::encrypt(user.id), winner_trend_id: duel.first_trend.id }
      expect(response).to have_http_status(403)
      expect(response.body).to eq('This duel does not belong to the given user')
    end
    it 'fails if used_id is not provided' do
      duel = create(:duel)
      patch :update, params: { id: duel.id, winner_trend_id: duel.first_trend.id }
      expect(response).to have_http_status(403)
    end
    it 'fails if used_id is invalid' do
      duel = create(:duel)
      patch :update, params: { id: duel.id, user_id: 1, winner_trend_id: duel.first_trend.id }
      expect(response).to have_http_status(403)
    end
    it 'fails when a duel was already patched' do
      duel = create(:answered_duel)
      patch :update, params: { id: duel.id, user_id: ApplicationHelper::encrypt(duel.user.id), winner_trend_id: duel.winner_trend_id }
      expect(response).to have_http_status(400)
    ...
    it 'fails when a winner is diferent from the two options' do
      duel1 = create(:duel)
      duel2 = create(:duel)
      patch :update, params: { id: duel2.id, user_id: ApplicationHelper::encrypt(duel2.user.id), winner_trend_id: duel1.first_trend.id }
      expect(response).to have_http_status(400)
    end
   it 'updates a duel' do
      duel = create(:duel)
      patch :update, params: { id: duel.id, user_id: ApplicationHelper::encrypt(duel.user.id), winner_trend_id: duel.first_trend.id }
